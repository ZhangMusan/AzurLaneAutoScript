    ...
    for row in source_rows:
        ...
        if polluted:
            ...
            if normalized in wiki_lib:
                ...
                best_key, fuzzy_score = _pick_best_fuzzy(normalized, wiki_keys)
                if best_key and fuzzy_score >= 0.86 and abs(len(best_key) - len(normalized)) <= 2:
                    final_name = wiki_lib[best_key]
                    method = 'fuzzy'
                elif best_key and int(level or 0) >= 100 and fuzzy_score >= 0.70 and abs(len(best_key) - len(normalized)) <= 4:
                    final_name = wiki_lib[best_key]
